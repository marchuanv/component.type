...
export class TypeRegisterEntry extends PropertiesTypeRegister {
    ...
    */
    constructor(targetClass, classExtensions = []) {
        ...
        const _classExtensions = Array.isArray(classExtensions) ? classExtensions : [classExtensions];
        if (!_classExtensions.every(c => Reflection.isClass(c))) {
            throw new Error(`The classExtensions argument contains non-classes.`);
        }
        ...
        if (properties.some(({ propertyType }) => Reflection.isEmptyString(propertyType))) {

...
export class Container {
    ...
    */
    get(property) {
        const { name, value, isPrivate } = getField.call(this, property);
        // if (isPrivate) {
        //     throw new Error(`${name} field is private`);
        // }
        return value;
...
function build(container = null) {
    if (container === null) {
        container = this;
    }
    const { fields } = privateBag.get(container);
    const privateFields = fields.filter(field => field.isPrivate);
    const instance = {};
    let data = [instance];
    for (const { Id, ctorArgsClass } of CtorArgsRegistry.metadata.filter(md => container.constructor === md.targetClass)) {
        instance.metadataId = Id.toString();
        instance.args = {};
        for (const field of privateFields) {
            let value = field.value;
            if (field.value instanceof Container) {
                const _data = build.call(container, field.value);

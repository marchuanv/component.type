...
export class Type extends Namespace{
    ...
    */
    constructor(target) {
        ...
        const propGetters = Reflect.ownKeys(targetClass.prototype)
            .map(key => Reflect.getOwnPropertyDescriptor(targetClass.prototype, key))
            .filter(p => p.get)
            .map(p => p.get);
        const propSetters = Reflect.ownKeys(targetClass.prototype)
            .map(key => Reflect.getOwnPropertyDescriptor(targetClass.prototype, key))
            .filter(p => p.set)
            .map(p => p.set);
        ...
        super.set({ name: 'properties' }, propMetadata);
    }
    /**
     * @returns { String }
    */
